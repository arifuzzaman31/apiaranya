    ...
    methods: {
        getShippingData(page = 1) {
            axios
                .get(
                    baseUrl +
                        `get-shipping-data?page=${page}&per_page=12&keyword=${this.search}`
                )
                .then((result) => {
        ...
        editShipping(item) {
            ...
            this.charge.country_name =
                item.country_name + " code " + item.country_code;
        ...
        addShppingCharge() {
            axios
                .post(baseUrl + `add-shipping-charge`, this.charge)
                .then((response) => {
        ...
        deleteShipping(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios
                        .delete(baseUrl + `remove-shipping-data/${id}`)
                        .then((response) => {
                            this.getShippingData();
                            this.successMessage(response.data);
                        })
                    ...
                    <div class="table-responsive" style="min-height: 60vh">
                        <table class="table table-bordered table-hover mb-4">
                            ...
                            <tbody
                                v-if="
                                    shipping_infos.data &&
                                    shipping_infos.data.length > 0
                                "
                            >
                                <template
                                    v-for="(item, index) in shipping_infos.data"
                                    :key="index"
                                >
                                    <tr>
                                        <td>{{ index + 1 }}</td>
                            ...
                            <form>
                                <div>
                                    ...
                                    <div class="modal-footer md-button">
                                        ...
                                        <button
                                            type="button"
                                            @click.prevent="addShppingCharge()"
                                            class="btn btn-info-a"
                                        >
                                            {{
                                                charge.id == ""
