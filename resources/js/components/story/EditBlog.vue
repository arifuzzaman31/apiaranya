...
export default {
    ...
    methods : {
        async updateBlog(){
            await axios.put(this.url+'update-blog',this.form).then(
                    response => {
                        if (response.data.status == "success") {
                            this.clearForm()
                        this.successMessage(response.data);
                        setTimeout(()=>{
                            window.location.href = this.url+"story-blog";
                        },1200)
                    }
                    }
                ). catch(e => {
        ...
        prepareData(){
            ...
            this.form.banner_image = this.blog.banner_image
            this.form.short_description = this.blog.short_description
            this.form.description = this.blog.description
            this.form.status = this.blog.status
        },
        selectImage(item){
